...
export default class ProductManager {
    constructor(){
        this.products = [];
        this.path = './src/manager/Products.json' // Ruta al archivo donde queremos grabar los datos.
    }
...
// Funcion que agrega un producto:
    addProduct = async (title, description, price, thumbnail, status, code, stock, id) =>{
        const product = {
            title,
            description,
            price,
        ...
        if (!product.title || !product.description || !product.price || !product.thumbnail || !product.status || !product.code || !product.stock) {
